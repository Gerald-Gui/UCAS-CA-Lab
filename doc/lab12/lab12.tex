...
\begin{document}
    ...
    这两个错误令人迷惑，也不知道从何处抓手，笔者决定重新检查状态机写法，上网搜索了三段式状态机写法，感觉未能找到类似笔者的写法：\texttt{next_state = next_state}，本意是让它保持在原先的状态不变，但是应该是不符合规范的，如果在\text{always (*)}块中时使用这种阻塞式写法，\texttt{next_state}并不知道自己该赋给自己什么值，因为可以将这种赋值方法下\texttt{next_state}看作一种提前性的随机而变的下一状态，因此对它的赋值都必须是确定的。
